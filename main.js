...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (track && prev && next) {
    ...
    prev.addEventListener('click', () => track.scrollBy({ left: -slideBy(), behavior: 'smooth' }));
    next.addEventListener('click', () => track.scrollBy({ left:  slideBy(), behavior: 'smooth' }));
  }
  ...
  if (track && dotsBox) {
    ...
    dots.forEach((dot, i) => {
      dot.addEventListener('click', () => {
        const x = slides[i].offsetLeft - (track.clientWidth - slides[i].clientWidth) / 2;
    ...
    track.addEventListener('scroll', () => {
      ...
      track._ticking = true;
      requestAnimationFrame(() => { updateFromScroll(); track._ticking = false; });
    });
  ...
  if (statsSection) {
    ...
    const io = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        counters.forEach(el => { if (el.dataset.done !== '1') animate(el); });
  ...
  // Animate unless user didn't opt into it AND section isn't forcing animation
const force = statsSection.hasAttribute('data-animate');
const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches && !force;
});
// ===== Skills: animate mirrored bars on first view =====
document.addEventListener('DOMContentLoaded', () => {
  const section = document.querySelector('#skills.skills-section');
  if (!section) return;
  ...
  function animateBar(bar) {
    ...
    function step(now) {
      ...
      bar.querySelector('.pct').textContent = pctNow + '%';
